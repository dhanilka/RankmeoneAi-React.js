...
export default function YtHashTagGenerator() {
  ...
  const generateMetaTitle = async () => {
    ...
    setTimeout(async () => {
      try {
        const response = await fetch("http://localhost:4000/instacap", {
          method: "POST",
          headers: {
    ...
    <div>
      ...
      <section className="topcontainerx text-center py-4">
        <div className="container">
          <h1 className="display-3 headrankmeone">Instagram Caption Generator</h1>
          <p className="lead headdesc">
          Generate modern caption for boost your audience.
          </p>
        </div>
      </section>
      {error && (
