...
export default function YtTagGenerator() {
  ...
  const generateMetaTitle = async () => {
    ...
    setTimeout(async () => {
      try {
        const response = await fetch("http://localhost:4000/twitcap", {
          method: "POST",
          headers: {
    ...
    <div>
      ...
      <section className="topcontainerx text-center py-4">
        <div className="container">
          <h1 className="display-3 headrankmeone">Twitter Caption Generator</h1>
          <p className="lead headdesc">
            Generate catchy Twitter captions to increase engagement.
          </p>
        </div>
      </section>
      {error && (
